                    ...
                    <div className="admin-panel animate-fade-in" style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
                            ...
                            <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--border-glass)', background: 'linear-gradient(to bottom right, #ffffff, #f8fafc)' }}>
                                <label className="admin-label">Total System Budget (All Providers)</label>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                                    ...
                                    {isUpdatingLimit ? (
                                        ...
                                        <button
                                            onClick={() => {
                                                const newLimit = prompt("Quick set Primary (OpenAI) limit in USD:", usageStats?.limit?.toString());
                                                if (newLimit && !isNaN(parseFloat(newLimit))) {
                                                    handleUpdateLimit(parseFloat(newLimit));
                                                }
                                            }}
                                            className="btn-secondary btn-sm"
                                            title="Quick Edit Primary Provider"
                                        >
                                ...
                                <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>
                                    Manage individual provider limits in the breakdown table below.
                                </p>
                            </div>

                            {/* Request Count Card */}
                            <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--border-glass)', background: 'linear-gradient(to bottom right, #ffffff, #f8fafc)' }}>
                                <label className="admin-label">Requests Count</label>
                                            ...
                                            {!usageStats?.detailed || usageStats.detailed.length === 0 ? (
                                                ...
                                                <tr key={int.id} style={{ borderBottom: '1px solid rgba(0,0,0,0.03)' }}>
                                                    ...
                                                    <td style={{ textAlign: 'right', padding: '1rem', color: 'var(--text-muted)' }}>
                                                        ...
                                                        <button
                                                            onClick={() => {
                                                                const newLimit = prompt(`Set daily limit for ${int.name}:`, int.limit.toString());
                                                                if (newLimit && !isNaN(parseFloat(newLimit))) {
                                                                    handleUpdateLimit(parseFloat(newLimit), int.id);
                                                                }
                                                            }}
                                                            style={{
                                                                background: 'none',
