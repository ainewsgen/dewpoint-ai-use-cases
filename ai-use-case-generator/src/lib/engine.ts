...
export async function generateOpportunities(companyData: CompanyData, promptDetails?: any): Promise<Opportunity[]> {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Capture specific error from backend (e.g. Budget Exceeded)
            const errData = await response.json().catch(() => ({}));
            fallbackReason = errData.error || `Server Error (${response.status})`;
            console.warn("AI Generation failed:", fallbackReason);
        }
    } catch (error) {
        console.error("AI Generation Error:", error);
        fallbackReason = error instanceof Error ? error.message : "Client Application Error";
        // Fallback to static
    }
